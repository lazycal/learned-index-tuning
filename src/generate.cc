...
vector<LBLookup<KeyType>> generate_equality_lookups(const vector<Row<KeyType>>& data,
                                                          const vector<T>& unique_keys,
                                                          const size_t num_lookups,
                                                          const double negative_lookup_ratio) {
  vector<LBLookup<KeyType>> lookups;
...
vector<LBLookup<KeyType>> generate_equality_lookups_from_trace(
...
template<class KeyType>
void print_equality_lookup_stats(const vector<LBLookup<KeyType>>& lookups) {
...
int main(int argc, char* argv[]) {
  ...
  switch (type) {
    case DataType::UINT32: {
      ...
      // Generate benchmarks.
      vector<LBLookup<uint32_t>> equality_lookups;
    ...
    case DataType::UINT64: {
      ...
      // Generate benchmarks.
      vector<LBLookup<uint64_t>> equality_lookups;
      if (filename.find("corp")!=std::string::npos) {
        // load the precomputed trace
        std::cout << "generating lookups from trace" << std::endl;
        std::string trace_name = filename;
